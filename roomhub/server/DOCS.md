...
scripts in that area, plus room media players. Saving updates the endpoint immediately. Visibility
...
Media players are delivered separately from dashboard tiles so they can be
opened from the endpoint audio control without consuming dashboard capacity.
