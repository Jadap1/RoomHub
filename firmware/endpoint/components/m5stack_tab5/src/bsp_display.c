 ...
 * 3: LCD ST7121, Touch ST7123-compatible
 */
static int bsp_get_board_version(void)
{
    ...
    if (i2c_master_probe(bsp_i2c_get_handle(), ESP_LCD_TOUCH_IO_I2C_ST7123_ADDRESS, 100) == ESP_OK) {
        ...
        if (esp_lcd_new_panel_io_i2c(bsp_i2c_get_handle(), &touch_io_config, &touch_io) == ESP_OK
            && esp_lcd_panel_io_rx_param(touch_io, 0x0000, &firmware_version, 1) == ESP_OK
            && firmware_version == 1) {
            ESP_LOGI(TAG, "Discovered board version 3 (LCD ST7121, touch firmware %u)", firmware_version);
            board_ver = 3;
        } else {
            ESP_LOGI(TAG, "Discovered board version 2 (LCD ST7123, touch firmware %u)", firmware_version);
            board_ver = 2;
        }
        if (touch_io != NULL) {
